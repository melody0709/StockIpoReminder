...
自动检查稳定版更新默认关闭，开启后最多每 6 小时做一次真实网络请求（成功结果进程内缓存 6 小时，命中不发请求）；独立的“发现更新后提前下载”开关也默认关闭，开启后只是提前把包下载好，安装仍需用户点击。应用关闭或 Windows 重启后，已验证的待安装更新会恢复为可点击的「更新」状态。更新过程不发送 Toast 或气泡；用户点过「稍后」的版本不再占用界面，出现更高版本时重新显示。
