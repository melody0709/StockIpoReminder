...
候选采集只覆盖提醒实际需要的范围，不下载全市场历史库：东方财富和北交所按申购日查询过去 60 天至未来 60 天；上交所使用其新股发行页面的“发行中”筛选；巨潮 IPO 接口保留自身提供的当前小结果集。需要分页的来源会读取过滤后声明的页数，最多读取 5 页；超过保护上限或声明数、明细数、可用身份数不一致时，来源进入 Warning，而不是把部分结果误判为完整覆盖。东方财富无数据时返回的明确空结果会按健康空结果处理。
...
当前 Rust 固定 fixture、SQLite 迁移、字段来源、公告链接关联、确认与 Outbox 恢复、同步调度、来源覆盖结论、退避/探测、备份、诊断、版本升级保护、安全卸载、更新清单、崩溃报告隐私约束、第二通知通道安全边界和有界采集查询回归测试共 87 项。
