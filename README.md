...
程序启动时自动同步一次；06:00–22:00 内普通日期默认每 30 分钟同步，当日存在未确认或待复核申购任务时默认每 10 分钟同步，并在申购日前一日 20:00、申购日 08:00 做定点核验。普通轮询和申购日轮询分别加入受控抖动，错过定点后会立即补做。提醒字段来自 Eastmoney、SSE、CNINFO、BSE 的结构化公开数据；程序只保存正式公告的标题、来源、发布时间和在线链接，不再自动下载或解析 PDF。
...
- 保存结构化字段来源和正式公告在线链接；需要时可用默认浏览器打开官方原文，程序不下载公告正文。
...
3. 交易所和披露网站的公告检索接口只用于保存标题、发布时间和官方 URL，方便人工打开核对。

沪市公告保留上交所官方检索，并优先使用巨潮资讯的沪市公告镜像元数据；镜像记录仍归入 `sse-announcement`，不会把备用通道误显示成另一个市场来源。程序只展示链接，不请求链接指向的 PDF 正文。
...
- 公告检索地址和保存的原文 URL 必须位于公告域名白名单。
- 公告结果优先保留正式发行公告，并排除律师意见、核查报告、保荐书、批复和招股书附录。
- 同一公告来源按整轮任务汇总健康状态：部分事件或备用路径异常显示“警告”，全部检索失败才显示“失败”。
- 采集器同时审计上游声明数量、返回明细数量和通过身份校验的数量；数量不一致的来源不能参与“完整覆盖”结论。
- 来源过期阈值会随用户配置的自动同步间隔调整；没有相关近期任务的公告来源不会仅因长期未调用而误报失败。
- 公告链接缺失或检索失败不会阻断结构化任务和提醒；存在字段冲突时仍明确要求人工核验。
...
announcements\            旧版本遗留的公告文件（新版本不再新增）
backups\                  自动备份和升级前备份
logs\                     轮转日志
...
设置页的“本地缓存与数据”可以直接打开当前数据目录；默认位置为 `%LocalAppData%\StockIpoReminder`，其中包含数据库、公告 PDF、日志、备份和诊断文件。该入口只打开目录，不会自动清理或修改文件。
...
当前 Rust 固定 fixture、SQLite 迁移、字段来源、公告链接关联、确认与 Outbox 恢复、同步调度、来源覆盖结论、退避/探测、备份、诊断、版本升级保护、安全卸载、更新清单、崩溃报告隐私约束和第二通知通道安全边界回归测试共 84 项。
...
旧 PDF Worker 的历史迁移和内存证据保留在 `.plan/feat/memory-footprint-pdf-rust.md`；当前产品设计基线见 `.plan/feat/windows-ipo-reminder.md`。
