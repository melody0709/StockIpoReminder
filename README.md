...
程序启动时自动同步一次；06:00–22:00 内普通日期默认每 30 分钟同步，当日存在未确认或待复核申购任务时默认每 10 分钟同步，并在申购日前一日 20:00、申购日 08:00 做定点核验。申购日前的同步只静默更新列表和详情，不产生提醒；普通轮询和申购日轮询分别加入受控抖动。错过定点后会在下一次允许的自动同步窗口补做，若已过 22:00 则最早延至次日 06:00，用户手动同步不受该窗口限制。提醒字段来自 Eastmoney、SSE、CNINFO、BSE 的结构化公开数据；程序只保存正式公告的标题、来源、发布时间和在线链接，不再自动下载或解析 PDF。
...
```text
StockIpoReminder-0.3.4-win-x64.msi
```
...
解压 `StockIpoReminder-0.3.4-win-x64-portable.zip` 后直接运行 `StockIpoReminder.exe`。便携版没有 Windows Installer 注册，因此设置页不会启用 MSI 卸载和自动更新入口；退出程序后直接删除便携文件即可，用户数据仍按下述规则独立保留。
...
当前本机生成的 `0.3.4` 基线没有接入正式 CA 证书，因此发布清单仍明确记录 `signed: false`，设置页会显示自动更新未配置。Windows SmartScreen 或安全软件仍可能显示“未知发布者”；请核对 `SHA256SUMS.txt`。正式签名和更新源部署见 `docs/release-signing-and-updates.md`。
