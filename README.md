...
程序启动时自动同步一次；06:00–22:00 内普通日期默认每 30 分钟同步，当日存在未确认或待复核申购任务时默认每 10 分钟同步，并在申购日前一日 20:00、申购日 08:00 做定点核验。普通轮询和申购日轮询分别加入受控抖动；错过定点后会在下一次允许的自动同步窗口补做，若已过 22:00 则最早延至次日 06:00，用户手动同步不受该窗口限制。提醒字段来自 Eastmoney、SSE、CNINFO、BSE 的结构化公开数据；程序只保存正式公告的标题、来源、发布时间和在线链接，不再自动下载或解析 PDF。

窗口隐藏到系统托盘后，完整界面和 SQLite 列表只在数据实际变化或窗口重新显示时刷新；调度心跳和健康状态使用低频合并写入。到期提醒仍独立按最多约 10 秒延迟检查，不会因为界面休眠而停止。
...
程序每小时独立检查每日数据库备份，默认保留最近 7 份；备份使用唯一临时文件、SQLite 完整性检查、写盘刷新和最终提交，失败时清理临时文件并保留既有备份。同步审计只在 SQLite 中保存来源、时间、条数、状态、结构指纹、响应哈希和脱敏错误，不保存上游接口正文；schema v9 会清除旧版已保存的接口正文，并在空闲页达到数据库四分之一时压缩文件；schema v10 将第二通知 Outbox 的身份扩展为“本地提醒 + 服务商”，保证切换服务商后仍能可靠入队。日志按日期保存、单日按大小分段并保留 14 天。诊断包 schema v4 默认不包含数据库、公告全文、原始接口响应或第二通知通道凭据，会脱敏 URL 查询参数、Cookie、Authorization、UNC/Windows 绝对路径、`--data-root`、临时目录和工作区绝对路径，并附带近期同步、来源/运维健康、本地与第二通知 Outbox、提醒日志、Windows Toast/AUMID 状态和最近的 Watchdog 崩溃报告。

主窗口和托盘会先建立，数据库备份、schema 迁移、完整性检查和必要的空间回收在后台完成；数据库就绪前，设置保存、任务确认和诊断导出会明确提示稍候，不会用默认值覆盖已有配置。设置或会话 JSON 损坏时会显示可诊断错误；保存设置时，SQLite 设置与提醒重算、DPAPI 凭据和 Run 注册状态按可回滚顺序更新，任一步失败都会尽力恢复保存前状态。
