...
程序默认启动后不打开主界面，直接驻留 Windows 系统托盘；双击托盘图标、使用托盘菜单、点击通知或再次启动程序时，才会显示主窗口。用户拉伸后的主窗口宽高会自动保存，下次打开程序并从托盘显示主界面时恢复；切换显示器或缩放比例后仍会限制在当前工作区内。主窗口隐藏期间，完整界面和 SQLite 列表只在数据实际变化或窗口重新显示时刷新；调度心跳和健康状态使用低频合并写入。到期提醒仍独立按最多约 10 秒延迟检查，不会因为界面休眠而停止。
...
自动检查稳定版更新默认关闭，开启后最多每 6 小时做一次真实网络请求（成功结果进程内缓存 6 小时，命中不发请求）；独立的“发现更新后提前下载”开关也默认关闭，开启后只是提前把包下载好，安装仍需用户点击。应用关闭或 Windows 重启后，已验证的待安装更新会恢复为可点击的「更新」状态。托盘右键菜单提供「打开发布页 (GitHub Release)」作为随时可用的手动入口。更新过程不发送 Toast 或气泡；用户点过「稍后」的版本不再占用界面，出现更高版本时重新显示。
