...
```text
StockIpoReminder-0.3.5-win-x64.msi
```
...
解压 `StockIpoReminder-0.3.5-win-x64-portable.zip` 后直接运行 `StockIpoReminder.exe`。便携版没有 Windows Installer 注册，因此设置页不会启用 MSI 卸载和自动更新入口；退出程序后直接删除便携文件即可，用户数据仍按下述规则独立保留。
...
当前本机生成的 `0.3.5` 基线没有接入正式 CA 证书，因此发布清单仍明确记录 `signed: false`，设置页会显示自动更新未配置。Windows SmartScreen 或安全软件仍可能显示“未知发布者”；请核对 `SHA256SUMS.txt`。正式签名和更新源部署见 `docs/release-signing-and-updates.md`。
