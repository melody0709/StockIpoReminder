...
- 剩余：第 12 节的第 3～7 项——修复显示问题后的 `0.3.9` 已发布为 stable/latest，等待从已安装的 `0.3.8` 完成一次真实的应用内升级（检查 → 下载 → 重启并更新，含 UAC 取消与失败恢复观察）。
- 记录日期：2026-09-09
- 当前应用版本：`0.3.7`，以 `Cargo.toml` 为唯一版本事实来源
...
- 自动检查按 24 小时节流，手动检查不受限制。
- 原有自动检查设置不会自动授权下载；新下载开关默认关闭。
- 同一版本只主动提醒一次，更新 Toast 不会进入股票详情。
...
4. 用户点击“重启并更新”后完成 MSI 升级并自动启动新版。
